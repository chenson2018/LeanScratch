...
theorem beta_shift_conserve  : ∀ {d c t1 t2}, (t1 →β t2) → Shifted d c t1 → Shifted d c t2 := by
  ...
  | reduce (β.reduce), (sapp (sabs s1) s2) => 
      have bs := @betaShifted2 d c 0
      simp at bs
      rw [Nat.add_comm] at bs
      exact bs s1 s2

theorem redex_shift_conserve : ∀ {d c t1 t2}, (t1 ↠β t2) → Shifted d c t1 → Shifted d c t2 := by
  intros d c t1 t2 redex s
  induction' redex
  case refl => assumption
  case tail a a_ih => exact beta_shift_conserve a a_ih

theorem para_shift_conserve  : ∀ {d c t1 t2}, (t1 ⇉  t2) → Shifted d c t1 → Shifted d c t2 := by
  intros d c t1 t2 para s
  exact redex_shift_conserve (para_to_redex para) s

theorem para_unshift {c d} {M M'} : (M ⇉ M') → Shifted d c M → (M.unshiftₙ c d ⇉ M'.unshiftₙ c d) := by
  intros para sM
  match para, sM with
  ...
  | Parallel.beta r2 r1, Shifted.sapp (Shifted.sabs s1) s2 => 
      ...
      case com_shift =>
        have bshift := @betaShifted2 d c 0
